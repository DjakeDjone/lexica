    ...
    <div>
        ...
        <div v-else>
            <div class="fill-in-formula">
                Formular: 
                <ContentRenderer :value="getMathFormula(mathFormulas[currentCardIdx]!)" />
            </div>
            <div>
                Items: 
                <ul>
                    <li v-for="item in getMathItems(mathFormulas[currentCardIdx]!)" :key="item">{{ item }}</li>
                </ul>
            </div>
            <!-- {{ mathFormulas[currentFormulaIdx] }} -->
            <div class="flex gap-4 items-center">
                <button class="btn btn-sm" @click="currentCardIdx = (currentCardIdx - 1 + mathFormulas.length) % mathFormulas.length">Previous</button>
                <span>{{ currentCardIdx + 1 }} / {{ mathFormulas.length }}</span>
                <button class="btn btn-sm" @click="currentCardIdx = (currentCardIdx + 1) % mathFormulas.length">Next</button>
            </div>
        </div>
